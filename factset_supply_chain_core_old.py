...
DATA_DIR_DEFAULT = "FactSetRevere/"  # folder with ent_scr_*.txt etc.
ENTITY_META_CSV_DEFAULT = "/factset_entityid_to_gvkey_with_company_name.csv"
...
def load_entity_metadata(path: str) -> Dict[str, Dict[str, Any]]:
    ...
    for _, row in df.iterrows():
        ...
        naics = str(int(naics_val)) if (naics_val not in ("", None) and not pd.isna(naics_val)) else ""
